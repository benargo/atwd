<?php
	if(is_uploaded_file($_FILES['csv']['tmp_name']) && substr($_FILES['csv']['name'], -4) == '.csv')
	{
		$csv = fopen($_FILES['csv']['tmp_name'], 'rt');
	}
	else
	{
		$csv = fopen('recorded_data.csv');
	}
	...
	// In this scenario we're only accepting data in the 12 months to June 2013.
	$crimes->setAttribute('year', $_POST['year']);
